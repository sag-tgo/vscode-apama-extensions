...
function extractItems(
  ...
  let rVal: vscode.CompletionItem[] = [];
  ...
  if (eventMatches) {
    for (let m in eventMatches) {
      ...
      if (!(symbol in rVal)) {
        if (symbol in Lang.workspaceLanguageDataMap) {
          rVal.push(Lang.workspaceLanguageDataMap[symbol].getCompletionItem());
        } else {
          ...
          rVal.push(temp.getCompletionItem());
        }
      } else {
      }
    }
...
export class EPLCompletionItemProvider
  ...
  public provideCompletionItems(
    ...
    theList = theList.concat(
      extractItems(document, actionRegex, vscode.CompletionItemKind.Function)
    );
    let varRegex = /\s*(integer|float|string|boolean)\s*([_A-Za-z][_A-Za-z0-9]*)?\s*\;/g;
    theList = theList.concat(
      extractItems(document, varRegex, vscode.CompletionItemKind.Variable)
    );
